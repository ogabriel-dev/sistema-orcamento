...
const DB = {
  ...
  read(key, fallback){
    ...
    }catch(e){ console.error('Erro ao ler storage', key, e); return fallback; }
  },
  write(key, value){
    try{ localStorage.setItem(key, JSON.stringify(value)); }
    catch(e){ console.error('Erro ao salvar storage', key, e); toast('Não foi possível salvar os dados.', 'error'); }
  },
  getClients(){ return this.read(this.KEYS.clients, []); },
  setClients(v){ this.write(this.KEYS.clients, v); },
  getBudgets(){ return this.read(this.KEYS.budgets, []); },
  setBudgets(v){ this.write(this.KEYS.budgets, v); },
  getSettings(){ return this.read(this.KEYS.settings, {
    companyName: '', logo: '', phone: '', email: '', cnpj: '', address: '',
    whatsappMsg: 'Olá {cliente}! Segue o orçamento nº {numero}, no valor de {valor}. Qualquer dúvida estou à disposição.'
  }); },
  setSettings(v){ this.write(this.KEYS.settings, v); },
  nextCounter(){
    let n = this.read(this.KEYS.counter, 0) + 1;
    this.write(this.KEYS.counter, n);
    return n;
  },
  peekCounter(){ return this.read(this.KEYS.counter, 0) + 1; }
};
...
function budgetNumberLabel(num){ return 'Nº ' + String(num).padStart(4, '0'); }

const STATUS_LABELS = {
  pendente: 'Pendente', enviado: 'Enviado', aprovado: 'Aprovado', recusado: 'Recusado', concluido: 'Concluído'
};
...
const PAGE_META = {
  ...
  'novo-orcamento': { title: 'Novo orçamento', subtitle: 'Monte um orçamento profissional' },
  ...
  configuracoes: { title: 'Configurações', subtitle: 'Dados usados nos orçamentos e PDFs' }
};
...
function renderClients(filter = ''){
  ...
  const list = state.clients
    .filter(c => !q || [c.name, c.company, c.phone, c.city].some(v => (v||'').toLowerCase().includes(q)))
    .sort((a,b) => a.name.localeCompare(b.name, 'pt-BR'));

  tbody.innerHTML = '';
  if(list.length === 0){
    empty.style.display = 'block';
    document.querySelector('#clientsTable').classList.toggle('is-hidden', state.clients.length === 0);
    if(state.clients.length > 0){ empty.querySelector('p').textContent = 'Nenhum cliente encontrado para essa pesquisa.'; empty.querySelector('button').style.display='none'; }
    return;
  }
  empty.style.display = 'none';
  document.querySelector('#clientsTable').classList.remove('is-hidden');

  list.forEach(c => {
    const count = state.budgets.filter(b => b.clientId === c.id).length;
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td class="cell-strong" data-label="Nome">${escapeHtml(c.name)}</td>
      <td data-label="Empresa">${escapeHtml(c.company) || '<span style="color:var(--text-tertiary)">—</span>'}</td>
      <td class="cell-mono" data-label="Telefone">${escapeHtml(c.whatsapp || c.phone) || '—'}</td>
      <td data-label="Cidade">${escapeHtml(c.city) || '—'}</td>
      <td data-label="Orçamentos">${count}</td>
  ...
  tbody.querySelectorAll('[data-edit]').forEach(b => b.addEventListener('click', () => openClientModal(b.dataset.edit)));
  tbody.querySelectorAll('[data-delete]').forEach(b => b.addEventListener('click', () => confirmDeleteClient(b.dataset.delete)));
}

document.getElementById('clientSearch').addEventListener('input', debounce(e => renderClients(e.target.value), 150));
document.getElementById('btnNewClient').addEventListener('click', () => openClientModal());
document.getElementById('btnNewClientEmpty').addEventListener('click', () => openClientModal());
...
function closeClientModal(){ clientModalOverlay.classList.remove('open'); }
...
clientModalOverlay.addEventListener('click', e => { if(e.target === clientModalOverlay) closeClientModal(); });
...
document.getElementById('clientSaveBtn').addEventListener('click', () => {
  ...
  if(!name){ toast('Informe o nome do cliente.', 'error'); document.getElementById('clientName').focus(); return; }
  if(!whatsapp){ toast('Informe o WhatsApp do cliente.', 'error'); document.getElementById('clientWhatsapp').focus(); return; }
  ...
  if(id){
    ...
    data.id = uid('cli'); data.createdAt = todayISO();
...
function populateClientSelects(){
  ...
  const sortedClients = [...state.clients].sort((a,b) => a.name.localeCompare(b.name, 'pt-BR'));

  const currentBudgetVal = budgetSel.value;
  budgetSel.innerHTML = '<option value="">Escolha um cliente cadastrado...</option>' +
    sortedClients.map(c => `<option value="${c.id}">${escapeHtml(c.name)}${c.company ? ' — ' + escapeHtml(c.company) : ''}</option>`).join('');
  ...
  filterSel.innerHTML = '<option value="">Todos os clientes</option>' +
    sortedClients.map(c => `<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('');
...
function newServiceRow(item){
  return { id: uid('srv'), desc: item?.desc || '', qty: item?.qty ?? 1, unitPrice: item?.unitPrice ?? 0 };
}
...
function renderServiceRows(){
  ...
  state.editingBudgetItems.forEach(item => {
    ...
    tr.innerHTML = `
      <td data-label="Descrição"><input type="text" class="desc-input" placeholder="Ex: Instalação elétrica" value="${escapeHtml(item.desc)}" data-field="desc"></td>
      <td data-label="Qtd."><input type="number" class="num-input" min="0" step="1" value="${item.qty}" data-field="qty"></td>
      <td data-label="Valor unit."><input type="number" class="num-input" min="0" step="0.01" value="${item.unitPrice}" data-field="unitPrice"></td>
      <td class="col-num cell-mono" data-label="Subtotal">${formatBRL(subtotal)}</td>
      <td class="cell-actions"><button type="button" class="icon-btn danger" title="Remover"><i class="fa-solid fa-trash"></i></button></td>`;
    ...
    tr.querySelector('.icon-btn.danger').addEventListener('click', () => {
      state.editingBudgetItems = state.editingBudgetItems.filter(x => x.id !== item.id);
      renderServiceRows();
      recalcBudgetTotals();
    });
  });
...
function recalcBudgetTotals(){
  const subtotal = state.editingBudgetItems.reduce((sum, it) => sum + (Number(it.qty)||0) * (Number(it.unitPrice)||0), 0);
  const discountVal = Number(document.getElementById('budgetDiscount').value) || 0;
  ...
  document.getElementById('sumDiscount').textContent = '- ' + formatBRL(discount);
...
['budgetDiscount','budgetDiscountType','budgetFee','budgetFeeType'].forEach(id => {
...
function resetBudgetForm(){
  ...
  document.getElementById('ticketNumber').textContent = budgetNumberLabel(DB.peekCounter());
  populateClientSelects();
  addServiceRow();
  recalcBudgetTotals();
}

document.getElementById('btnCancelBudget').addEventListener('click', () => goToPage('historico'));

document.getElementById('budgetForm').addEventListener('submit', e => {
  e.preventDefault();
  const clientId = document.getElementById('budgetClient').value;
  if(!clientId){ toast('Selecione um cliente para o orçamento.', 'error'); return; }
  const items = state.editingBudgetItems.filter(it => it.desc.trim() !== '');
  if(items.length === 0){ toast('Adicione ao menos um serviço com descrição.', 'error'); return; }
...
function loadBudgetForEdit(id){
  ...
  document.getElementById('budgetDiscountType').value = b.discountType || 'value';
  ...
  document.getElementById('budgetValidUntil').value = formatDateInput(b.validUntil);
  document.getElementById('budgetStatus').value = b.status || 'pendente';
  document.getElementById('budgetNotes').value = b.notes || '';
  document.getElementById('ticketNumber').textContent = budgetNumberLabel(b.number);
  recalcBudgetTotals();
}
...
function duplicateBudget(id){
  ...
  DB.setBudgets(state.budgets);
  toast(`Orçamento duplicado como ${budgetNumberLabel(copy.number)}.`, 'success');
  renderHistory();
  closeViewModal();
}
...
function getFilteredBudgets(){
  ...
  let list = state.budgets.map(b => ({ ...b, client: state.clients.find(c => c.id === b.clientId) }));
...
function renderHistory(){
  ...
  tbody.innerHTML = '';
  document.getElementById('historyTable').classList.toggle('is-hidden', list.length === 0);
  empty.style.display = list.length ? 'none' : 'block';

  list.forEach(b => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td class="cell-mono" data-label="Nº">${budgetNumberLabel(b.number)}</td>
      <td class="cell-strong" data-label="Cliente">${escapeHtml(b.client?.name || 'Cliente removido')}</td>
      <td data-label="Data">${formatDate(b.createdAt)}</td>
      <td data-label="Validade">${formatDate(b.validUntil)}</td>
      <td class="cell-mono" data-label="Valor">${formatBRL(b.total)}</td>
      <td data-label="Status"><span class="badge badge-${b.status}">${STATUS_LABELS[b.status]}</span></td>
      <td class="cell-actions">
        <div class="row-actions">
  ...
  tbody.querySelectorAll('[data-edit]').forEach(b => b.addEventListener('click', () => loadBudgetForEdit(b.dataset.edit)));
  tbody.querySelectorAll('[data-delete]').forEach(b => b.addEventListener('click', () => confirmDeleteBudget(b.dataset.delete)));
}

['historySearch'].forEach(id => document.getElementById(id).addEventListener('input', debounce(renderHistory, 150)));
['filterStatus','filterClient','sortBy'].forEach(id => document.getElementById(id).addEventListener('change', renderHistory));

function confirmDeleteBudget(id){
  const b = state.budgets.find(x => x.id === id);
  if(!b) return;
  openConfirm('Excluir orçamento', `Tem certeza que deseja excluir o orçamento ${budgetNumberLabel(b.number)}? Esta ação não pode ser desfeita.`, () => {
...
document.getElementById('btnExportCsv').addEventListener('click', () => {
  const list = getFilteredBudgets();
  if(list.length === 0){ toast('Nenhum orçamento para exportar.', 'error'); return; }
  const header = ['Numero','Cliente','Empresa','Data','Validade','Status','Subtotal','Total','FormaPagamento'];
  ...
  a.href = url; a.download = `orcamentos_${new Date().toISOString().slice(0,10)}.csv`;
...
function openViewModal(id){
  ...
  document.getElementById('viewModalTitle').textContent = `Orçamento ${budgetNumberLabel(b.number)}`;
  ...
  const discountDisplay = b.discountType === 'percent' ? `${b.discount}%` : formatBRL(b.discount);
...
function closeViewModal(){ viewModalOverlay.classList.remove('open'); state.viewingBudgetId = null; }
...
document.getElementById('btnEditFromView').addEventListener('click', () => {
  const id = state.viewingBudgetId; closeViewModal(); loadBudgetForEdit(id);
});
document.getElementById('btnDuplicate').addEventListener('click', () => duplicateBudget(state.viewingBudgetId));

document.getElementById('btnWhatsapp').addEventListener('click', () => {
  const b = state.budgets.find(x => x.id === state.viewingBudgetId);
  if(!b) return;
  const client = state.clients.find(c => c.id === b.clientId);
  if(!client || !client.whatsapp){ toast('Este cliente não possui WhatsApp cadastrado.', 'error'); return; }
  const phoneDigits = client.whatsapp.replace(/\D/g, '');
  const phoneWithCountry = phoneDigits.startsWith('55') ? phoneDigits : '55' + phoneDigits;

  let msg = state.settings.whatsappMsg || 'Olá {cliente}! Segue o orçamento nº {numero}, no valor de {valor}.';
  msg = msg
...
document.getElementById('btnPdf').addEventListener('click', () => generatePdf(state.viewingBudgetId));
...
function generatePdf(id){
  ...
  if(s.logo){
    try{
      const fmt = (s.logo.match(/^data:image\/(\w+);/) || [,'PNG'])[1].toUpperCase().replace('JPG','JPEG');
      doc.setFillColor(255,255,255);
      doc.roundedRect(14, (HEADER_H - LOGO_SIZE) / 2, LOGO_SIZE, LOGO_SIZE, 2, 2, 'F');
      doc.addImage(s.logo, fmt, 14, (HEADER_H - LOGO_SIZE) / 2, LOGO_SIZE, LOGO_SIZE, undefined, 'FAST');
    }catch(err){ console.error('Não foi possível inserir a logo no PDF', err); }
  }
  ...
  doc.setFontSize(9);
  doc.text('Emitido em ' + formatDate(b.createdAt), 196, 21, { align: 'right' });
  doc.text('Válido até ' + formatDate(b.validUntil), 196, 26, { align: 'right' });
  ...
  doc.autoTable({
    ...
    body: b.items.map(it => [
      it.desc, String(it.qty), formatBRL(it.unitPrice), formatBRL((Number(it.qty)||0)*(Number(it.unitPrice)||0))
    ]),
    theme: 'grid',
  ...
  doc.text(`Subtotal:  ${formatBRL(b.subtotal)}`, totalsX, y, { align: 'right' }); y += 5.5;
  doc.text(`Desconto:  - ${discountDisplay}`, totalsX, y, { align: 'right' }); y += 5.5;
  doc.text(`Taxa:  + ${feeDisplay}`, totalsX, y, { align: 'right' }); y += 7;
  ...
  if(b.paymentMethod){ doc.text(`Forma de pagamento: ${b.paymentMethod}`, 14, y); y += 6; }
  if(b.deadline){ doc.text(`Prazo de execução: ${b.deadline}`, 14, y); y += 6; }
  if(b.notes){
...
function closeConfirm(){ confirmModalOverlay.classList.remove('open'); state.confirmCallback = null; }
document.getElementById('confirmModalClose').addEventListener('click', closeConfirm);
document.getElementById('confirmCancelBtn').addEventListener('click', closeConfirm);
confirmModalOverlay.addEventListener('click', e => { if(e.target === confirmModalOverlay) closeConfirm(); });
...
function renderDashboard(){
  ...
  document.getElementById('statTotal').textContent = formatBRL(budgets.reduce((s,b) => s + (b.total||0), 0));
  document.getElementById('statApproved').textContent = budgets.filter(b => b.status === 'aprovado').length;
  document.getElementById('statPending').textContent = budgets.filter(b => b.status === 'pendente').length;
  document.getElementById('statRejected').textContent = budgets.filter(b => b.status === 'recusado').length;

  const recent = [...budgets].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 6);
  const tbody = document.getElementById('recentBudgetsBody');
  tbody.innerHTML = '';
  document.getElementById('dashboardEmpty').style.display = budgets.length ? 'none' : 'block';
  document.getElementById('recentBudgetsTable').classList.toggle('is-hidden', budgets.length === 0);
  ...
  recent.forEach(b => {
    ...
    tr.innerHTML = `
      <td class="cell-mono" data-label="Nº">${budgetNumberLabel(b.number)}</td>
      <td class="cell-strong" data-label="Cliente">${escapeHtml(client?.name || 'Cliente removido')}</td>
      <td data-label="Data">${formatDate(b.createdAt)}</td>
      <td class="cell-mono" data-label="Valor">${formatBRL(b.total)}</td>
      <td data-label="Status"><span class="badge badge-${b.status}">${STATUS_LABELS[b.status]}</span></td>
      <td class="cell-actions"><button class="icon-btn" title="Visualizar" data-view="${b.id}"><i class="fa-solid fa-eye"></i></button></td>`;
    tbody.appendChild(tr);
  });
  tbody.querySelectorAll('[data-view]').forEach(b => b.addEventListener('click', () => openViewModal(b.dataset.view)));
}
...
function renderReports(){
  ...
  const countByMonth = months.map(m => state.budgets.filter(b => {
    const d = new Date(b.createdAt); return `${d.getFullYear()}-${d.getMonth()}` === m.key;
  }).length);
  const revenueByMonth = months.map(m => state.budgets.filter(b => {
    const d = new Date(b.createdAt); return `${d.getFullYear()}-${d.getMonth()}` === m.key && b.status !== 'recusado';
  ...
  charts.byMonth = new Chart(document.getElementById('chartByMonth'), {
    type: 'bar',
    data: { labels: months.map(m=>m.label), datasets: [{ label: 'Orçamentos', data: countByMonth, backgroundColor: c.primary, borderRadius: 6, maxBarThickness: 34 }] },
    options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{ x:{ grid:{ display:false }, ticks: commonTicks }, y:{ beginAtZero:true, ticks:{ ...commonTicks, precision:0 }, grid: commonGrid } } }
  });

  const statusKeys = ['pendente','enviado','aprovado','recusado','concluido'];
  const statusColors = [c.amber, c.blue, c.green, c.red, c.violet];
  const statusCounts = statusKeys.map(k => state.budgets.filter(b => b.status === k).length);
  charts.byStatus = new Chart(document.getElementById('chartByStatus'), {
    type: 'doughnut',
    data: { labels: statusKeys.map(k => STATUS_LABELS[k]), datasets: [{ data: statusCounts, backgroundColor: statusColors, borderWidth: 0 }] },
    options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:'bottom', labels:{ color: c.text, boxWidth:10, padding:14, font:{family:'Inter', size:11.5} } } }, cutout: '62%' }
  });

  charts.revenue = new Chart(document.getElementById('chartRevenue'), {
    type: 'line',
    data: { labels: months.map(m=>m.label), datasets: [{ label:'Faturamento', data: revenueByMonth, borderColor: c.primary, backgroundColor: c.primary + '26', fill:true, tension:.35, pointRadius:4, pointBackgroundColor: c.primary }] },
    options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{ x:{ grid:{ display:false }, ticks: commonTicks }, y:{ beginAtZero:true, ticks:{ ...commonTicks, callback:v=>'R$ '+v }, grid: commonGrid } } }
  });

  const clientTotals = state.clients.map(cl => ({
    name: cl.name, count: state.budgets.filter(b => b.clientId === cl.id).length
  })).filter(x => x.count > 0).sort((a,b) => b.count - a.count).slice(0, 6);
  charts.topClients = new Chart(document.getElementById('chartTopClients'), {
    type: 'bar',
    data: { labels: clientTotals.map(x=>x.name), datasets: [{ label:'Orçamentos', data: clientTotals.map(x=>x.count), backgroundColor: c.primary, borderRadius:6, maxBarThickness:22 }] },
    options: { indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{ x:{ beginAtZero:true, ticks:{ ...commonTicks, precision:0 }, grid: commonGrid }, y:{ grid:{ display:false }, ticks: commonTicks } } }
  });
}
...
function updateLogoPreview(logo){
  ...
  preview.innerHTML = logo ? `<img src="${logo}" alt="Logo">` : '<i class="fa-solid fa-store"></i>';
  const sidebarLogo = document.getElementById('sidebarCompanyLogo');
  sidebarLogo.innerHTML = logo ? `<img src="${logo}" alt="Logo">` : '<i class="fa-solid fa-store"></i>';
}

document.getElementById('settingsLogo').addEventListener('change', e => {
  const file = e.target.files[0];
  if(!file) return;
  if(file.size > 1024 * 1024){ toast('A imagem deve ter no máximo 1MB.', 'error'); return; }
  const reader = new FileReader();
  reader.onload = () => { updateLogoPreview(reader.result); state.settings._tempLogo = reader.result; };
  reader.readAsDataURL(file);
});
document.getElementById('removeLogo').addEventListener('click', () => { updateLogoPreview(''); state.settings._tempLogo = ''; });
...
document.getElementById('settingsForm').addEventListener('submit', e => {
  ...
  document.getElementById('sidebarCompanyName').textContent = state.settings.companyName || 'Minha Empresa';
